...
describe("EthereumICPBridge", function () {
  ...
  describe("Swap Creation", function () {
    ...
    const timelock = Math.floor(Date.now() / 1000) + 7200; // 2 hours from now (more than 1 hour minimum)
    ...
    it("Should fail with timelock too long", async function () {
      const longTimelock = Math.floor(Date.now() / 1000) + 86400 * 2; // 2 days
      
      await expect(
        bridge.connect(user1).createSwap(icpRecipient, hashlock, longTimelock, {
          value: ethers.parseEther("0.1")
        })
  ...
  describe("Swap Completion", function () {
    ...
    it("Should fail when timelock expired", async function () {
      // Fast forward time past the timelock (2 hours + 1 second)
  ...
  describe("Swap Refund", function () {
    ...
    beforeEach(async function () {
      // Create a swap with timelock 2 hours from now
      const swapAmount = ethers.parseEther("0.1");
      const currentTime = await ethers.provider.getBlock("latest").then(block => block.timestamp);
      const timelock = currentTime + 7200; // 2 hours from current block time
